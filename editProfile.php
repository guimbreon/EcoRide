...
$stmt = $conn->prepare("SELECT nome, NIF, telemovel, email, ft_perfil, pass FROM Utilizadores WHERE email = ?");
if (!$stmt) {
    die("Erro ao preparar a consulta: " . $conn->error . " - Query: SELECT nome, NIF, telemovel, email, ft_perfil, pass FROM Utilizadores WHERE email = ?");
}

// Check if $conn is valid before preparing the statement
if (!$conn) {
    die("Erro: Conexão com o banco de dados não foi estabelecida.");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ((!is_strong_password($pass) || mysqli_real_escape_string($conn, $pass) != $pass) && $pass != '') {
    ...
    if (isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["error"] == 0) {
        ...
        // If no file is uploaded, keep the current profile picture
        $target_file = $user['ft_perfil'];
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("UPDATE Utilizadores SET email=?, nome=?, nif=?, telemovel=?, pass=?, ft_perfil=? WHERE email=?");
        
        $_SESSION['user']['email'] = $nome;
        $_SESSION['user']['nome'] = $email;
        if (!$stmt) {
            die("Erro ao preparar a atualização: " . $conn->error);
        }
        $current_email = $user['email'];
        $stmt->bind_param("sssssss", $email, $nome, $nif, $tlmv, $hashed_pass, $target_file, $current_email);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Erro ao atualizar o perfil: " . $stmt->error;
        }
        $stmt->close();
    }
}
                    ...
                    <form action="editProfile.php" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
                        <div class="d-flex justify-content-center">
                            <div class="mb-3" style="width: 100%; max-width: 550px;">
                                ...
                                <input type="text" class="form-control form-control-sm" id="nome" name="nome" value="<?php echo htmlspecialchars($user['nome']); ?>" required>
                        ...
                        <div class="d-flex justify-content-center">
                            <div class="mb-3" style="width: 100%; max-width: 550px;">
                                ...
                                <input type="text" class="form-control form-control-sm" id="email" name="email" value="<?php echo htmlspecialchars($user['email']); ?>" required>
                                <div class="invalid-feedback">
                        ...
                        <div class="d-flex justify-content-center">
                            <div class="mb-3" style="width: 100%; max-width: 550px;">
                                ...
                                <input type="text" class="form-control form-control-sm" id="tlmv" name="tlmv" value="<?php echo htmlspecialchars($user['telemovel']); ?>" required>
                                <div class="invalid-feedback">
                        ...
                        <div class="d-flex justify-content-center">
                            <div class="mb-3" style="width: 100%; max-width: 550px;">
                                ...
                                <input type="text" class="form-control form-control-sm" id="nif" name="nif" value="<?php echo htmlspecialchars($user['NIF']); ?>" required>
                                <div class="invalid-feedback">
                                    Por favor, insira um NIF valido.
                                </div>
                            </div>
                        </div>
                        <div class="d-flex justify-content-center">
                            <div class="mb-3" style="width: 100%; max-width: 550px;">
                                ...
                                <div class="input-group">
                                    <input type="password" class="form-control form-control-sm" id="pass" name="pass" placeholder="Insira a nova palavra-passe ou deixe em branco para manter a atual">
