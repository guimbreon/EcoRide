...
<html lang="en-US" dir="ltr">
                    ...
                    .then(data => {
                      if (Array.isArray(data.ids)) {
                        if (data.ids.length > 0) {
                          // Scroll para a seção de destino se houver reservas
                          const section = document.getElementById('destination');
                          if (section) {
                            section.scrollIntoView({ behavior: 'smooth' });
                          }
                        }
                        console.log('Reserva IDs atualizados:', data.ids);
                        data.ids.forEach(id => {
                           ...
                           if (chatBtn) {
                            ...
                            if (cardDiv) {
                              ...
                              .then(cardHtml => {
                                ...
                                cardDiv.remove();
                                // Insere o novo card no início de #viagensCondutor
  ...
  <!-- <section> Viagens Passageiros teste begin ============================-->
      <?php
                  ...
                  fetch('scripts/reservaUpdates.php?lastCheck=' + lastCheck)
                    ...
                    .then(data => {
                      if (Array.isArray(data.ids)) {
                        ...
                        console.log('Reserva IDs atualizados2:', data.ids);
                        data.ids.forEach(id => {
                          // Procura o botão de chat com o idRes correspondente
                          const chatBtn = document.querySelector(`a[href="chat.php?idRes=${id}"]`);
                          if (chatBtn) {
