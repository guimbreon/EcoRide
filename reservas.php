  ...
  <main class="main" id="top" >
    ...
    <section class="pt-5" id="destination">
      <div class="container">
            ...
            <form method="GET" action="reservas.php" class="d-flex mb-4">
              ...
              <select class="form-control me-2" name="localidade" style="width: 25%;">
                ...
                while ($row = $result->fetch_assoc()) {
                  echo '<option value="' . htmlspecialchars($row['localidade'], ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($row['localidade'], ENT_QUOTES, 'UTF-8') . '</option>';
                }
                $conn->close();
                ?>
              </select>
              ...
              <select class="form-control me-2" name="preco" style="width: 25%;">
                <option value="all">Preço</option>
                <?php
                include "scripts/abreconexao.php";
                $sql = "SELECT DISTINCT preco FROM Viagens ORDER BY preco";
                $result = $conn->query($sql);
                while ($row = $result->fetch_assoc()) {
                  echo '<option value="' . htmlspecialchars($row['preco'], ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($row['preco'], ENT_QUOTES, 'UTF-8') . '</option>';
                }
                $conn->close();
                ?>
              </select>
              <input type="date" class="form-control me-2" name="data" style="width: 25%;" />
              <a href="reservaMapa.php" style="margin-left: 10px; margin-right:12px;" title="Reservar viagem no mapa">
                <img src="assets/img/globe.png" alt="Mapa" style="width: 40px; height: 40px;">
              </a>
        ...
        <?php
        if (
          (isset($_GET['localidade']) && !empty($_GET['localidade'])) ||
          (isset($_GET['hora']) && !empty($_GET['hora'])) ||
          (isset($_GET['data']) && !empty($_GET['data'])) ||
          (isset($_GET['preco']) && !empty($_GET['preco']))
        ):
        ?>
        <div class="row">
          <?php
          ...
          $sql = "SELECT V.id, V.data_hora, V.lugares, V.preco,
            L1.nome AS origem_nome, L2.nome AS destino_nome, L2.localidade AS localidade, V.lugares
            FROM Viagens V
            JOIN Local L1 ON V.origem = L1.id
            JOIN Local L2 ON V.destino = L2.id
            WHERE V.data_hora > NOW() AND V.lugares > 0";

          if (!empty($localidade) && $localidade !== "all") {
            $localidade = $conn->real_escape_string($localidade);
            $sql .= " AND L2.localidade LIKE '$localidade' ";
          }

          if (!empty($hora) && $hora !== "all") {
          ...
          if (!empty($preco) && $preco !== "all") {
            $preco = $conn->real_escape_string($preco);
            $sql .= " AND V.preco = '$preco' ";
          }
          $conn->set_charset("utf8mb4");
          $result = $conn->query($sql);
          ...
          if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              $idReserva = null;
              $viagemId = $row["id"];
              $sqlReserva = "SELECT id FROM Reserva WHERE viagem_id = $viagemId LIMIT 1";
              $resultReserva = $conn->query($sqlReserva);
              if ($resultReserva && $resultReserva->num_rows > 0) {
                $reservaRow = $resultReserva->fetch_assoc();
                echo "<script>alert('ENTROU AKI');</script>";
                $idReserva = $reservaRow['id'];
              }
              echo renderViagemCard($row["data_hora"], $row["origem_nome"], $row["destino_nome"], $row["lugares"], $row["localidade"], $row["id"], $row["preco"], $idReserva);
            }
          } else {
            echo "<script>alert('Não foram encontradas viagens para os critérios selecionados.');</script>";
          }
    ...
    <section>
      ...
      <form action="scripts/addReserva.php" method="post" class="text-start mx-auto" style="max-width: 400px;">
        ...
        <div class="mb-3">
          ...
          <input type="number" id="numLugares" name="numLugares" class="form-control" min="1" max="10" value="1" required>
        </div>
        </div>
