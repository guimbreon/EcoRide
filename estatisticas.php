  ...
  <body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light fixed-top py-5 d-block" data-navbar-on-scroll="data-navbar-on-scroll">
      ...
      <!-- <section> Viagens Passageiros begin ============================-->
            ...
            // Query to fetch the count of trips for each location
            $query = "
            SELECT 
              l.nome AS localName, 
              COUNT(*) AS tripCount
            FROM 
              Viagens v
            INNER JOIN 
              Local l ON v.origem = l.id OR v.destino = l.id
            GROUP BY 
              l.nome
            ORDER BY 
              tripCount DESC
            ...
            document.addEventListener('DOMContentLoaded', function () {
              // Dados para o gráfico de locais usados nas viagens
              const locaisUsadosData = <?php echo json_encode($tripCounts); ?>;
              const locaisUsadosLabels = <?php echo json_encode($localNames); ?>;

              const locaisUsadosCtx = document.getElementById('locaisUsados').getContext('2d');
              const locaisUsadosChart = new Chart(locaisUsadosCtx, {
                type: 'bar',
                options: {
                  responsive: true,
