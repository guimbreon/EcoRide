...
if ($row = $result->fetch_assoc()) {
    ...
    // Gera o HTML do card
    $cardHtml = renderViagemCard($date, $origem, $destino, $localidade, $preco, $pontoRecolha, $idReserva, true);

    // Envia um script para inserir o card no elemento com id 'viagensCondutor'
    echo "<script>
        const row = document.querySelector('#viagensCondutor');
        if(row) {
            row.insertAdjacentHTML('beforeend', " . json_encode($cardHtml) . ");
        }
    </script>";
