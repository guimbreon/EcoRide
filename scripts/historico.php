...
if (isset($_POST['enviar_avaliacao'])) {
  $reserva_id = $_POST['idRes'];
  $comentario = $_POST['comentario'];
  $rating = $_POST['rating'];
  $idPass = $user['id'];

  $stmtAvaliacao = $conn->prepare("UPDATE Reservas SET comentario = ?, rating = ? WHERE id = ? AND idPass = ?");
  $stmtAvaliacao->bind_param("siii", $comentario, $rating, $reserva_id, $idPass);
  if ($stmtAvaliacao->execute()) {
      ...
      <section style="padding-top: 7rem;">
        ...
        <div class="container">
          ...
          <div class="row">
                    ...
                    include "scripts/viagemPassagisRated.php";
                    if (!empty($viagens)) {
                        foreach ($viagens as $viagem) {
                          $isRated = !empty($viagem["comentario"]) && !empty($viagem["avaliacao"]);
                          
                          if(!$isRated){
                            echo renderViagemCard($viagem['data'], $viagem['origem'], $viagem['destino'], $viagem['localidade'], $viagem['preco'], $viagem['pontoRecolha'],$viagem['idRes'], false, $isRated);
                          }else{
                            echo renderViagemCardRated($viagem['data'], $viagem['origem'], $viagem['destino'], $viagem['localidade'], $viagem['preco'], $viagem['pontoRecolha'],$viagem['idRes'], false, $isRated);
                          }
