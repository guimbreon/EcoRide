...
function renderViagemCard($date, $origem, $destino, $localidade, $preco, $pontoRecolha, $idReserva = null, $hasNotificacao = false, $isRated = false, $comentario, $rate) {
  ...
  if (strpos($isHistoricoPage, 'historico.php') !== false and !$isRated) {
  ...
  $notificacaoBadge = '';
  // Exibe o badge de notificação temporariamente (por exemplo, por 5 segundos)
  if ($hasNotificacao) {
    $notificacaoBadge = '
      <span id="notificacao-badge-' . $idReserva . '" style="
        position: absolute;
        top: 10px;
        right: 18px;
        width: 18px;
    ...
    <div class="card overflow-hidden shadow" style="position: relative;">
      ...
      <div class="card-body py-4 px-3">
        <div class="d-flex flex-column justify-content-between mb-3">
          ...
          ' . ($isRated ? '<div class="mb-2"><strong>Avaliação:</strong> ' . htmlspecialchars($rate) . ' ⭐</div>
          <div class="mb-2"><strong>Comentário:</strong> ' . htmlspecialchars($comentario) . '</div>' : '') . '
